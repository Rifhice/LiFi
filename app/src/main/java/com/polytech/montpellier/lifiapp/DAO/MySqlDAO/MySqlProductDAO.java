...
public class MySqlProductDAO extends ProductDAO {
    ...
    public void getProductDiscounts(final Product product, final ResponseHandler response) throws DAOException {
        ...
        final DiscountDAO discountdao = AbstractDAOFactory.getFactory(AbstractDAOFactory.MYSQL_DAO_FACTORY).getDiscountDAO();
        final ArrayList<Discount> discounts = new ArrayList<Discount>();
        Helper.getInstance().GET("http://81.64.139.113:1337/api/Product/" + product.getId() + "/Discounts/", new ResponseHandler() {
            @Override
            public void onSuccess(Object object) {
                if (object instanceof JSONArray) {
                    JSONArray array = (JSONArray) object;
